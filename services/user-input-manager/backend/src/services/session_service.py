...
class SessionService:
    ...
    async def submit_feedback(
        ...
        await self._session_repo.update(
            session, tm_ticket_title=result.suggested_title or session.tm_ticket_title
        )
    ...
    async def approve_and_create_ticket(
        ...
        iterations = await self._iter_repo.list_for_session(session_id)
        last_assistant = next((it for it in reversed(iterations) if it.role == "assistant"), None)
