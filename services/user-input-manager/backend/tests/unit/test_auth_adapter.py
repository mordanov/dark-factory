"""Unit tests for KeycloakValidator (src.core.auth_adapter).
...
JWKS_URL = "http://keycloak:8080/realms/dark-factory/protocol/openid-connect/certs"
...
def _make_jwks_response(secret: str = TEST_SECRET) -> dict:
    """Return a minimal JWKS-like response with a symmetric key for HS256 mocking."""
    return {
        "keys": [
            {
                "kty": "oct",
...
# ---------------------------------------------------------------------------
# T070: test_auth_adapter.py — 8 tests
# ---------------------------------------------------------------------------
...
async def test_keycloak_mode_fetches_jwks(monkeypatch):
    ...
    with patch("src.core.auth_adapter.httpx.AsyncClient") as mock_client_cls:
        ...
        validator = KeycloakValidator()
        # Ensure fresh cache on this instance
        ...
        try:
            ...
            pass  # We only care that httpx.get was called
...
async def test_jwks_refreshed_after_ttl_expired(monkeypatch):
    ...
    with (
        ...
        # First call at t=0, second call at t=400 (past 300s TTL)
...
async def test_algorithm_confusion_rejected(monkeypatch):
    ...
    # Craft a token with alg:none — header.payload.empty-signature
    header = (
...
async def test_unrecognised_auth_mode_raises(monkeypatch):
    ...
    validator = KeycloakValidator()
    # AUTH_MODE is read during verify() — the error surfaces then
