...
vi.mock('../src/api/orchestrator', () => ({
  pendingApi: { list: vi.fn() },
  jobsApi: { trigger: vi.fn() },
}))
...
describe('PendingTicketsPage', () => {
  beforeEach(async () => {
    const { pendingApi, jobsApi } = await import('../src/api/orchestrator')
    vi.mocked(pendingApi.list).mockResolvedValue(
      { data: { tickets: mockTickets, total: 2 } } as never
    )
    vi.mocked(jobsApi.trigger).mockResolvedValue(
      { data: { id: 'job-1', status: 'pending' } } as never
    )
  })
  ...
  it('trigger button calls jobsApi.trigger', async () => {
    ...
    await waitFor(() => expect(jobsApi.trigger).toHaveBeenCalledWith('t-1', 'p-1'))
  })
