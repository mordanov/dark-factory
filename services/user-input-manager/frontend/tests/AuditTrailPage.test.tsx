...
describe('AuditTrailPage', () => {
  beforeEach(async () => {
    const { auditApi } = await import('../src/api/orchestrator')
    vi.mocked(auditApi.list).mockResolvedValue(
      { data: { items: mockEntries, total: 2 } } as never
    )
  })
  ...
  it('triggers search on Enter key', async () => {
    ...
    await waitFor(() => expect(auditApi.list).toHaveBeenCalledWith('t-enter'))
  })

  it('shows empty state when no entries', async () => {
    const { auditApi } = await import('../src/api/orchestrator')
    vi.mocked(auditApi.list).mockResolvedValueOnce({ data: { items: [], total: 0 } } as never)
    wrap(<AuditTrailPage />)
