...
const auditItems = [
  ...
  },
  {
    id: 'a-2', job_id: 'job-1', ticket_id: 't-1', project_id: 'p-1',
    action: 'BLOCK', from_state: 'specification', to_state: null,
    assigned_agent: 'project_manager', blocked_reason: 'Gate triage_complete failed',
...
describe('JobDetailModal', () => {
  beforeEach(async () => {
    ...
    vi.mocked(orchestratorApi.getAuditTrail).mockResolvedValue(
      { data: { items: auditItems, total: 2 } } as never
    )
  })
  ...
  it('shows state transitions', async () => {
    wrap()
    await waitFor(() => {
      // "triage" is a text node inside a span that also contains " → specification",
      // so multiple ancestor elements match /triage/ — use getAllByText to avoid the throw
