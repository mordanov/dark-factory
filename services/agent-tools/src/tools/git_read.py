...
async def read_file(
    ...
    try:
        ...
        return build_error(
            tool, "TIMEOUT", f"read_file timed out after {s.git_read_timeout_seconds}s", True, t0
        )
        ...
        return build_error(
            tool, "FILE_NOT_FOUND", f"Path '{exc}' not found at ref '{ref}'", False, t0
        )
...
async def list_files(
    ...
    try:
        ...
        return build_error(
            tool, "TIMEOUT", f"list_files timed out after {s.git_read_timeout_seconds}s", True, t0
        )
    except _FileNotFoundError as exc:
...
def _grep(
    repo: git.Repo, query: str, path_filter: str, case_sensitive: bool, max_results: int
...
async def search_code(
    ...
    try:
        ...
        return build_error(
            tool,
            "SEARCH_TIMEOUT",
            f"search_code timed out after {s.git_read_timeout_seconds}s",
            True,
            t0,
        )

    return build_success(
        tool, SearchCodeResult(matches=matches, truncated=truncated).model_dump(), t0
    )
...
def _diff(
    repo: git.Repo, base_ref: str, head_ref: str, path_filter: str
) -> tuple[str, list[str], DiffStats]:
    ...
    return (
        raw_diff,
        files_changed,
        DiffStats(additions=additions, deletions=deletions, files=num_files),
    )
...
async def get_diff(
    ...
    try:
        ...
        return build_error(
            tool, "TIMEOUT", f"get_diff timed out after {s.git_read_timeout_seconds}s", True, t0
        )
    except _RefNotFoundError as exc:
        return build_error(tool, "REF_NOT_FOUND", f"Ref '{exc}' not found", False, t0)
