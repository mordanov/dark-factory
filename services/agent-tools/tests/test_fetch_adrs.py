...
_SAMPLE_ADRS = [
    {
        ...
        "status": "accepted",
        "summary": "Chose PostgreSQL over MongoDB for relational data",
    ...
    },
    {
        ...
        "title": "Auth via JWT",
        "status": "accepted",
        "summary": "Use HS256 JWT for auth service tokens",
    ...
    },
    {
        ...
        "status": "proposed",
        "summary": "Considering Redis for session caching",
...
async def test_fetch_adrs_accepted_filter(settings):
    with respx.mock(base_url=_BASE) as mock:
        mock.get("/api/v1/memory/proj-1/adrs").mock(
            return_value=httpx.Response(
                200,
                json={"adrs": [a for a in _SAMPLE_ADRS if a["status"] == "accepted"]},
            )
        )
...
async def test_fetch_adrs_domain_filter(settings):
    with respx.mock(base_url=_BASE) as mock:
        ...
        result = await fetch_adrs(
            "proj-1", status_filter="all", domain_filter="auth", settings=settings
        )
    assert result.success is True
...
async def test_fetch_adrs_empty_list(settings):
    with respx.mock(base_url=_BASE) as mock:
        mock.get("/api/v1/memory/proj-empty/adrs").mock(
            return_value=httpx.Response(
                200,
                json={"adrs": []},
            )
