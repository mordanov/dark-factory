...
async def test_list_pending_returns_pending_only(db):
    repo = JobRepository(db)
    j1 = await repo.create(
        job_type="orchestrate", ticket_id="t-2", project_id="p-1", triggered_by="u", payload={}
    )
    j2 = await repo.create(
        job_type="orchestrate", ticket_id="t-3", project_id="p-1", triggered_by="u", payload={}
    )
...
async def test_mark_done(db):
    repo = JobRepository(db)
    job = await repo.create(
        job_type="distill", ticket_id="t-4", project_id="p-1", triggered_by="system", payload={}
    )
...
async def test_mark_failed(db):
    repo = JobRepository(db)
    job = await repo.create(
        job_type="orchestrate", ticket_id="t-5", project_id="p-1", triggered_by="system", payload={}
    )
...
async def test_has_running_job(db):
    ...
    job = await repo.create(
        job_type="orchestrate", ticket_id="t-99", project_id="p-1", triggered_by="u", payload={}
    )
    await repo.mark_running(job.id)
...
async def test_list_all_with_filters(db):
    repo = JobRepository(db)
    await repo.create(
        job_type="orchestrate", ticket_id="filter-t", project_id="p-1", triggered_by="u", payload={}
    )
