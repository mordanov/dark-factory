"""Unit tests for KeycloakServiceClient (src.core.keycloak_client) — orchestrator.
...
TOKEN_ENDPOINT = "http://keycloak:8080/realms/dark-factory/protocol/openid-connect/token"
...
async def test_token_refreshed_30s_before_expiry():
    ...
    with patch("httpx.AsyncClient") as mock_client_cls, patch("time.monotonic") as mock_time:
        ...
        # First call at t=0: token is None → skip check, set expires_at=0+300=300
        # Second call at t=280: check 280 < 300-30=270 → False → refresh triggered
...
async def test_keycloak_error_raises_upstream_error():
    ...
    with patch("httpx.AsyncClient") as mock_client_cls:
        ...
        from src.core.keycloak_client import UpstreamError
