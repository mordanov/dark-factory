...
async def test_pending_returns_ticket_updated_after_last_orchestrator_run(
    ...
    ticket = await _ticket(
        db_session, project, user, fsm_status="triage", last_orchestrator_run=past
    )
    await db_session.commit()
...
async def test_pending_excludes_ticket_not_updated_since_last_run(db_session: AsyncSession):
    ...
    ticket = await _ticket(
        db_session, project, user, fsm_status="triage", last_orchestrator_run=future
    )
    await db_session.commit()
...
async def test_pending_cursor_pagination_no_overlap_no_gaps(db_session: AsyncSession):
    ...
    page1 = await get_pending_tickets(db_session, project_id=project.id, limit=3, after_cursor=None)
