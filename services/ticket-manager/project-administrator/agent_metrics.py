...
def resolve_html_path(value: str | None) -> Path:
    if value:
        return Path(value).expanduser().resolve()
...
def init_db(conn: sqlite3.Connection) -> None:
    ...
    conn.execute("CREATE INDEX IF NOT EXISTS idx_events_model ON events(model_used)")
...
def aggregate_events(events: Iterable[sqlite3.Row]) -> dict[str, object]:
    by_agent: dict[str, dict[str, int]] = defaultdict(
        lambda: {"seconds": 0, "tokens": 0, "events": 0, "unknown_tokens": 0}
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "record":
        ...
        print(f"Recorded event for {event.agent_name} ({event.feature_name}) at {event.timestamp}")
        return 0
