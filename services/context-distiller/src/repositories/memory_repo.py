...
class MemoryRepository:
    ...
    async def archive_then_write(self, project_id: str, yaml_content: str, ticket_id: str) -> None:
    ...
    async def get_adrs(self, project_id: str, status_filter: str = "accepted") -> list[dict]:
    ...
    async def update_adr_status(self, project_id: str, adr_id: str, new_status: str) -> dict:
        ...
        if new_status not in _VALID_TRANSITIONS.get(current_status, set()):
            raise ConflictError(f"Invalid status transition: {current_status} → {new_status}")
