    ...
    try:
        async with httpx.AsyncClient(timeout=settings.orchestrator_timeout_seconds) as c:
            resp = await c.request(
                ...
                json=body, params={k: v for k, v in (params or {}).items() if v is not None},
            )
...
async def pending_tickets(
    ...
    return await _proxy("GET", "/api/v1/jobs/pending-tickets",
                        _token(request), params={"project_id": project_id})
...
async def list_jobs(
    ...
    return await _proxy("GET", "/api/v1/jobs", _token(request),
...
async def audit_trail(
    ...
    return await _proxy("GET", f"/api/v1/audit/{ticket_id}", _token(request),
...
async def project_memory(
    request: Request, project_id: str, _: User = Depends(get_current_user)
):
...
async def project_adrs(
    ...
    return await _proxy("GET", f"/api/v1/memory/{project_id}/adrs",
                        _token(request), params={"status": status})
