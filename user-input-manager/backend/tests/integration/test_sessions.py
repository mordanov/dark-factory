...
@pytest.mark.asyncio
async def test_create_session_new_project(client, app, db, auth_headers, mock_tm_client, mock_llm_result):
    app.dependency_overrides[get_session_service] = lambda: SessionService(db, mock_tm_client)

    with patch("src.services.session_service.refine_prompt", AsyncMock(return_value=mock_llm_result)):
        resp = await client.post("/api/v1/sessions", headers=auth_headers, json={
...
@pytest.mark.asyncio
async def test_create_session_existing_project(client, app, db, auth_headers, mock_tm_client, mock_llm_result):
    app.dependency_overrides[get_session_service] = lambda: SessionService(db, mock_tm_client)
...
async def test_get_iterations(client, app, db, auth_headers, mock_tm_client, mock_llm_result):
    ...
    with patch("src.services.session_service.refine_prompt", AsyncMock(return_value=mock_llm_result)):
        create_resp = await client.post("/api/v1/sessions", headers=auth_headers, json={
            "session_type": "new_project",
...
@pytest.mark.asyncio
async def test_submit_feedback_not_approved(client, app, db, auth_headers, mock_tm_client, mock_llm_result):
    app.dependency_overrides[get_session_service] = lambda: SessionService(db, mock_tm_client)
...
@pytest.mark.asyncio
async def test_submit_feedback_approved(client, app, db, auth_headers, mock_tm_client, mock_llm_result):
    app.dependency_overrides[get_session_service] = lambda: SessionService(db, mock_tm_client)
...
@pytest.mark.asyncio
async def test_approve_creates_ticket(client, app, db, auth_headers, mock_tm_client, mock_llm_result):
    app.dependency_overrides[get_session_service] = lambda: SessionService(db, mock_tm_client)
