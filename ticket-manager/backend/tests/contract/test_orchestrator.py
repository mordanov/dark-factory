...
@pytest.mark.asyncio
async def test_pending_ticket_includes_required_fields(client: AsyncClient, db_session: AsyncSession):
    user = await _user(db_session)
    project = await _project(db_session, user)
    await _ticket(db_session, project, user)
    await db_session.commit()
...
@pytest.mark.asyncio
async def test_fsm_patch_unknown_ticket_returns_404(
    client: AsyncClient, db_session: AsyncSession
):
    admin = await _user(db_session, role=UserRole.administrator)
    project = await _project(db_session, admin)
...
async def test_fsm_patch_response_includes_all_fsm_fields(
    ...
    data = resp.json()
    for field in (
        "fsm_status", "blocked_reason", "brainstorm_round",
        "assigned_agent", "override", "override_reason",
        "last_orchestrator_run", "orchestrator_errors",
    ):
...
@pytest.mark.asyncio
async def test_batch_fsm_status_returns_correct_map(
    client: AsyncClient, db_session: AsyncSession
):
    admin = await _user(db_session, role=UserRole.administrator)
    project = await _project(db_session, admin)
...
@pytest.mark.asyncio
async def test_full_ticket_fsm_defaults_when_unset(
    client: AsyncClient, db_session: AsyncSession
):
    user = await _user(db_session)
    project = await _project(db_session, user)
