...
async def test_multiple_events_returned_in_chronological_order(db_session: AsyncSession):
    ...
    events = [
        AuditEventCreate(event="ADVANCE", actor="orchestrator", from_state="backlog", to_state="triage"),
        AuditEventCreate(event="BLOCK", actor="orchestrator", from_state="triage", to_state="BLOCKED", details="Missing spec"),
        AuditEventCreate(event="ADVANCE", actor="orchestrator", from_state="BLOCKED", to_state="triage"),
    ]
    for body in events:
...
def test_audit_service_exposes_no_update_or_delete():
    ...
    mutation_names = [f for f in public_functions if any(
        verb in f for verb in ("update", "delete", "remove", "patch", "modify")
    )]
