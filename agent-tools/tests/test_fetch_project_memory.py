...
async def test_fetch_memory_happy_path(settings):
    with respx.mock(base_url=_BASE) as mock:
        mock.get("/api/v1/memory/proj-1").mock(return_value=httpx.Response(
            200,
            json={"content": "project: test\nkey: value\n", "last_ticket_id": "T-42", "version": 1},
        ))
...
async def test_fetch_memory_truncation(settings):
    ...
    with respx.mock(base_url=_BASE) as mock:
        mock.get("/api/v1/memory/proj-big").mock(return_value=httpx.Response(
            200,
...
async def test_fetch_memory_empty_ticket_id(settings):
    with respx.mock(base_url=_BASE) as mock:
        mock.get("/api/v1/memory/proj-empty").mock(return_value=httpx.Response(
            200,
