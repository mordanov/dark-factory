...
from src.schemas.schemas import (
    AgentBriefing, DecisionDetail, OrchestratorDecision, TmTicket
)
...
async def test_process_job_advance(db, svc, mock_tm, mock_doc_store):
    ...
    job = await repo.create(
        job_type="orchestrate", ticket_id="t-1", project_id="p-1",
        triggered_by="human", payload={},
    )

    with patch("src.services.orchestrator_service.call_orchestrator_llm",
...
async def test_process_job_needs_estimation_no_llm(db, mock_doc_store):
    ...
    mock_tm = MagicMock()
    mock_tm.get_ticket_full = AsyncMock(return_value={
        "id": "t-estim", "project_id": "p-1", "title": "T",
        "description": "D", "ticket_type": "feature",
        "tags": ["needs-estimation"], "fsm_status": "backlog",
        "blocked_reason": None, "brainstorm_round": 0, "assigned_agent": None,
        "override": False, "override_reason": None,
        "dependencies": [], "subtasks": [], "created_at": None, "updated_at": None,
    })
    ...
    repo = JobRepository(db)
    job = await repo.create(
        job_type="orchestrate", ticket_id="t-estim", project_id="p-1",
        triggered_by="human", payload={},
    )
