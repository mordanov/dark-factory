...
class DocumentStore:
    ...
    async def save_memory(
        ...
        if existing:
            ...
            history_count = await self._db[_MEMORY_HISTORY_COL].count_documents({"project_id": project_id})
            if history_count > keep:
                oldest_docs = await self._db[_MEMORY_HISTORY_COL].find(
                    {"project_id": project_id}
                ).sort("version", 1).limit(history_count - keep).to_list(None)
