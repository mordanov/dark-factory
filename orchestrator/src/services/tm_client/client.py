...
class TicketManagerClient:
    ...
    async def get_pending_tickets(self, project_id: str | None = None, limit: int = 50) -> list[dict]:
    ...
    async def manage_tags(self, project_id: str, ticket_id: str, *, add: list[str] = (), remove: list[str] = ()) -> dict:
    ...
    async def get_fsm_status_batch(self, ticket_ids: list[str]) -> dict[str, dict]:
        ...
        data = await self._request("POST", "/api/tickets/fsm-status-batch", json={"ticket_ids": ticket_ids})
